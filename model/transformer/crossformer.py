...
class CrossFormer(nn.Module):
    def __init__(
        ...
        *,
        ...
        dim = cast_tuple(dim, 2)
        depth = cast_tuple(depth, 2)
        global_window_size = cast_tuple(global_window_size, 2)
        local_window_size = cast_tuple(local_window_size, 2)
        cross_embed_kernel_sizes = cast_tuple(cross_embed_kernel_sizes, 2)
        cross_embed_strides = cast_tuple(cross_embed_strides, 2)

        assert len(dim) == 2
        assert len(depth) == 2
        assert len(global_window_size) == 2
        assert len(local_window_size) == 2
        assert len(cross_embed_kernel_sizes) == 2
        assert len(cross_embed_strides) == 2
